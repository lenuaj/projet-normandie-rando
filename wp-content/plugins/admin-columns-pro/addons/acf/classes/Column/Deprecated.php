...
class Deprecated extends AC\Column {
	...
	public function __construct() {
		$this
			...
			->set_label( __( 'ACF (Deprecated)', 'lenuaj-admin-columns' ) )
			->set_group( 'acf' );
	}
	...
	protected function register_settings() {
		...
		$message->set_label( __( 'Update Message', 'lenuaj-admin-columns' ) );
		$message->set_message(
			sprintf( '<div class="acf-deprecated-message">%s %s<div class="acf-deprecated-message-field">%s<br>%s<br>%s<br>%s<br>%s<br>%s</div></div>',
				__( 'This ACF column could not be updated from the v2 to v3 version.', 'lenuaj-admin-column' ),
				sprintf( __( 'Read more about this in %s.' ),
					sprintf( '<a href="%s">%s</a>', $documentation_url, __( 'our documentation', 'lenuaj-admin-column' ) )
				),
				sprintf( '<strong>%s</strong>', __( 'ACF Field', 'lenuaj-admin-columns' ) ),
				sprintf( __( 'Field Label: %s', 'lenuaj-admin-column' ), sprintf( '<em>%s</em>', $acf_field['label'] ) ),
				sprintf( __( 'Field Name: %s', 'lenuaj-admin-column' ), sprintf( '<em>%s</em>', $acf_field['name'] ) ),
				sprintf( __( 'Field Type: %s', 'lenuaj-admin-column' ), sprintf( '<em>%s</em>', $acf_field['type'] ) ),
				sprintf( __( 'Field Key: %s', 'lenuaj-admin-column' ), sprintf( '<em>%s</em>', $hash ) ),
				sprintf( __( 'Field Group: %s', 'lenuaj-admin-column' ), sprintf( '<a href="%s">%s</a>', $edit_group_url, sprintf( '<em>%s</em>', $acf_group['title'] ) ) )
			)
		);
