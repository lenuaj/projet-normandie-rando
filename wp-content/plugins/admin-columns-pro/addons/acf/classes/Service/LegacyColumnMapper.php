	...
	public function render_deprecated_columns_notice( ListScreen $list_screen ) {
		if ( ! $list_screen->is_read_only() && $this->has_deprecated_columns( $list_screen ) ) {
			...
			<div class="aca-acf-deprecated-columns-notice">
				<p>
					<?= __( 'These settings contain at least one deprecated ACF column.', 'lenuaj-admin-columns' ); ?>
					<?= __( 'You can automatically update these columns to the latest version.', 'lenuaj-admin-columns' ); ?>
				</p>
				<form method="post">
					<?= ( new UpdateDeprecatedNonce() )->create_field(); ?>
					<input type="hidden" name="migrate_list_screen_id" value="<?= $list_screen->get_id()->get_id() ?>"/>
					<button type="submit" class="button" name="action" value="aca-acf-map-legacy-list-screen"><?= __( 'Update Now', 'lenuaj-admin-columns' ); ?></button>
				</form>
			</div>
			<?php
