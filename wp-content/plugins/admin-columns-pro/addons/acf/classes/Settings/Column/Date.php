...
class Date extends AC\Settings\Column\Date {
	...
	protected function get_custom_format_options() {
		$label = __( 'ACF Date Format', 'lenuaj-admin-columns' );

		$options = [
			'acf' => $this->get_html_label(
				$label,
				$this->get_acf_date_format(),
				sprintf( __( "%s uses the %s from it's field settings.", 'lenuaj-admin-columns' ), $label, '"' . __( 'Display Format', 'lenuaj-admin-columns' ) . '"' )
			),
		];
