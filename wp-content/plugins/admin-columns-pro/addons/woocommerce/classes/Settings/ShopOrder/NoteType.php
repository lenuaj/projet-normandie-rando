...
class NoteType extends AC\Settings\Column {
	...
	public function create_view() {
		$select = $this->create_element( 'select' )
		               ...
		               ->set_options( [
			               ''                  => __( 'All Notes', 'lenuaj-admin-columns' ),
			               self::SYSTEM_NOTE   => __( 'System Notes', 'lenuaj-admin-columns' ),
			               self::PRIVATE_NOTE  => __( 'Private Notes', 'lenuaj-admin-columns' ),
			               self::CUSTOMER_NOTE => __( 'Notes to Customer', 'lenuaj-admin-columns' ),
		               ] );

		return new AC\View( [
			'label'   => __( 'Note Type', 'lenuaj-admin-columns' ),
			'setting' => $select,
		] );
	}
