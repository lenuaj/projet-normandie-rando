...
class ShippingMethodType extends AC\Settings\Column {
	...
	public function create_view() {
		$select = $this->create_element( 'select' )
		               ->set_options( [
			               self::METHOD_TITLE => __( 'Method Label', 'lenuaj-admin-columns' ),
			               self::METHOD_ID    => __( 'Method Type', 'lenuaj-admin-columns' ),
		               ] );

		return new AC\View( [
			'label'   => __( 'Shipping Method Type', 'lenuaj-admin-columns' ),
			'setting' => $select,
		] );
	}
