...
class Notes extends AC\Column implements ACP\Editing\Editable, AC\Column\AjaxValue, ACP\Search\Searchable
{
    ...
    public function get_value($id)
    {
        ...
        return ac_helper()->html->get_ajax_modal_link(
            sprintf(_n('%d note', '%d notes', $count, 'lenuaj-admin-columns'), $count),
            [
    ...
    public function get_ajax_value($id)
    {
        ...
        foreach ($this->get_order_notes($id) as $note) {
            ...
            if ($this->is_customer_note($note)) {
                $class = '-customer-note';
                $type = __('Note To Customer', 'lenuaj-admin-columns');
            } elseif ($this->is_system_note($note)) {
                $class = '-system-note';
                $type = __('System', 'lenuaj-admin-columns');
            } elseif ($this->is_private_note($note)) {
                $class = '-private-note';
                $type = __('Private', 'lenuaj-admin-columns');
            }
