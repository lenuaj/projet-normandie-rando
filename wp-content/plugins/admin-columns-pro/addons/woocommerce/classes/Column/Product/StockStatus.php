    ...
    implements ACP\Sorting\Sortable, ACP\Editing\Editable, ACP\Search\Searchable
{
    ...
    public function get_value($post_id)
    {
        ...
        switch ($this->get_raw_value($post_id)) {
            case 'instock' :
                return ac_helper()->icon->yes(__('In stock', 'lenuaj-admin-columns'));
            case 'outofstock' :
                return ac_helper()->icon->no(__('Out of stock', 'lenuaj-admin-columns'));
            case 'onbackorder' :
                return ac_helper()->icon->dashicon(
                    [
                        'icon'    => 'backup',
                        'class'   => 'yellow',
                        'tooltip' => __('On backorder', 'lenuaj-admin-columns'),
                    ]
                );
            default :
                return $this->get_empty_char();
