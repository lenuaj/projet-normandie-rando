...
class SalePrice extends Price {
	...
	public function save() {
		...
		if ( $price <= 0 ) {
			return new WP_Error( 'invalid-price', __( 'Sale price can not be zero or lower.', 'lenuaj-admin-columns' ) );
		}

		if ( $price >= (float) $this->product->get_regular_price() ) {
			return new WP_Error( 'invalid-price', __( 'Sale price can not be higher than the regular price.', 'lenuaj-admin-columns' ) );
		}
