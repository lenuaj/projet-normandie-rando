...
final class TableScreen implements Registerable
{
    ...
    public function table_scripts_editing($list_screen)
    {
        ...
        wp_localize_script('acp-editing-table', 'acp_woocommerce_i18n', [
            'woocommerce' => [
                ...
                'clear_sale_price'          => __('Clear Sale Price', 'lenuaj-admin-columns'),
                'decimals'                  => __('Decimals', 'lenuaj-admin-columns'),
                'decrease_by'               => __('Decrease by', 'lenuaj-admin-columns'),
                'downloadable'              => __('Downloadable', 'woocommerce'),
                'height'                    => __('Height', 'woocommerce'),
                'in_stock'                  => __('In stock', 'woocommerce'),
                'increase_by'               => __('Increase by', 'lenuaj-admin-columns'),
                ...
                'regular'                   => __('Regular', 'lenuaj-admin-columns'),
                'regular_price'             => __('Regular Price', 'lenuaj-admin-columns'),
                'replace'                   => __('Replace', 'lenuaj-admin-columns'),
                'sale'                      => __('Sale', 'woocommerce'),
                'sale_price'                => __('Sale price', 'woocommerce'),
                'sale_from'                 => __('Sale from', 'lenuaj-admin-columns'),
                'sale_to'                   => __('Sale To', 'lenuaj-admin-columns'),
                'set_new'                   => __('Set New', 'lenuaj-admin-columns'),
                'set_sale_based_on_regular' => __(
                    'Set the sale price based on the regular price',
                    'lenuaj-admin-columns'
                ),
                'schedule'                  => __('Schedule', 'woocommerce'),
                'schedule_from'             => _x('From', 'schedule', 'lenuaj-admin-columns'),
                'schedule_to'               => _x('To', 'schedule', 'lenuaj-admin-columns'),
                'scheduled'                 => __('Scheduled', 'lenuaj-admin-columns'),
                ...
                'rounding_none'             => __('No Rounding', 'lenuaj-admin-columns'),
                'rounding_up'               => __('Round up', 'lenuaj-admin-columns'),
                'rounding_down'             => __('Round down', 'lenuaj-admin-columns'),
                'rounding_example'          => __('Example:', 'lenuaj-admin-columns'),
                'virtual'                   => __('Virtual', 'woocommerce'),
                'delete_note'               => __('Delete note', 'lenuaj-admin-columns'),
    ...
    public function set_editing_role_group(string $group, string $role): string
    {
        if (in_array($role, ['customer', 'shop_manager'])) {
            $group = __('WooCommerce', 'lenuaj-admin-columns');
        }
    ...
    public function add_quick_action_variation($actions, $post)
    {
        ...
        if ($link) {
            $actions['variation'] = ac_helper()->html->link($link, __('View Variations', 'lenuaj-admin-columns'));
        }
    ...
    public function add_quick_link_variation($column, $post_id)
    {
        ...
        $label = ac_helper()->html->tooltip(
            ...
            __('View Variations', 'lenuaj-admin-columns')
        );
