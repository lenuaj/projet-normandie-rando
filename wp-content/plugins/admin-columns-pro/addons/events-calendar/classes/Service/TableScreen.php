    ...
    public function add_events_filter_vars($list_screen): void
    {
        ...
        switch ($input->date) {
            case 'future':
                ...
                $date = __('upcoming', 'lenuaj-admin-columns');

                break;
            case 'past':
                $this->filter_on_past_events();
                $date = __('previous', 'lenuaj-admin-columns');

                break;
            default:
                $date = __('all', 'lenuaj-admin-columns');
        }

        // General notice
        $notice[] = sprintf(
            __('Filtering on %s: Showing %s events from %s.', 'lenuaj-admin-columns'),
        ...
        $notice[] = sprintf(
            ...
            sprintf(__('Return to %s', 'lenuaj-admin-columns'), $post_type_object->labels->name)
        );
        ...
        $notice[] = sprintf(
            ...
            __('Remove this filter', 'lenuaj-admin-columns')
        );
