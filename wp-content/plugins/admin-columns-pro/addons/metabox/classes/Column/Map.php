...
class Map extends Column {
	...
	public function format_single_value( $value, $id = null ) {
		...
		$parts = [
			sprintf( '%s: %s', __( 'Latitude', 'lenuaj-admin-columns' ), $value['latitude'] ),
			sprintf( '%s: %s', __( 'Longitude', 'lenuaj-admin-columns' ), $value['longitude'] ),
			sprintf( '%s: %s', __( 'Zoom', 'lenuaj-admin-columns' ), $value['zoom'] ),
		];
