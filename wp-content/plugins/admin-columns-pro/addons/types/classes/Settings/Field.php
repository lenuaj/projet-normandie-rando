...
class Field extends AC\Settings\Column {
	...
	public function create_view() {
		...
		$select
			->set_no_result( sprintf( __( 'No %s fields available.', 'lenuaj-admin-columns' ), __( 'Types', 'lenuaj-admin-columns' ) ) )
		...
		$view = new View( [
			...
			'description' => sprintf( __( 'Select your %s field.', 'lenuaj-admin-columns' ), __( 'Types', 'lenuaj-admin-columns' ) ) . '<em>' . __( 'Type', 'lenuaj-admin-columns' ) . ': ' . $this->get_types_field() . '</em>',
			'setting'     => $select,
		] );
