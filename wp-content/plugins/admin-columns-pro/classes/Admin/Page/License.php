...
class License implements Asset\Enqueueables, Renderable, RenderableHead
{
    ...
    private function render_section_updates(): View
    {
        ...
        $view = new View([
            'title'                      => __('Updates', 'lenuaj-admin-columns'),
            'content'                    => $content,
    ...
    private function get_inline_notice(): ?string
    {
        ...
        if ( ! $permissions->has_usage_permission()) {
            $description = __('Enter your license code to receive automatic updates.', 'lenuaj-admin-columns');
        }

        if ( ! $permissions->has_updates_permission()) {
            $description = sprintf(
                __('Enter your license key to %s.', 'lenuaj-admin-columns'),
                sprintf(
                    '<strong>%s</strong>',
                    __('unlock all settings', 'lenuaj-admin-columns')
                )
            );
        }
    ...
    {
        $page = __('network settings page', 'lenuaj-admin-columns');
        ...
        $content = sprintf(
            __('The license can be managed on the %s.', 'lenuaj-admin-columns'),
            $page
        );
        ...
        $view = new View([
            'title'   => __('License', 'lenuaj-admin-columns'),
