    ...
    protected function delete($id, array $args = []): void
    {
        ...
        if ( ! current_user_can('delete_users') || ! current_user_can('delete_user', $id)) {
            throw new RuntimeException(
                __('You do not have permissions to delete this user.', 'lenuaj-admin-columns')
            );
        }

        if (get_current_user_id() === $id) {
            throw new RuntimeException(__('The current user can not be deleted.', 'lenuaj-admin-columns'));
        }
        ...
        if ($reassign_user === $id) {
            throw new RuntimeException(__('The assigned user can not be deleted.', 'lenuaj-admin-columns'));
        }

        $result = wp_delete_user($id, $reassign_user);

        if (false === $result) {
            throw new RuntimeException(__('User does not exists.', 'lenuaj-admin-columns'));
        }
    }
