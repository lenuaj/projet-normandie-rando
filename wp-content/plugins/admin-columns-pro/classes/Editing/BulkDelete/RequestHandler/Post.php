...
class Post extends RequestHandler
{
    ...
    private function restore_post($id): void
    {
        if ( ! $this->user_can_delete($id)) {
            throw new RuntimeException(__('You have no permission to restore this item.', 'lenuaj-admin-columns'));
        }
        ...
        if ('trash' !== $post->post_status) {
            throw new RuntimeException(
                sprintf(__('%s is not in Trash.', 'lenuaj-admin-columns'), $this->post_type->labels->singular_name)
            );
        }
    ...
    private function delete_post(string $post_type, int $id, bool $force_delete): void
    {
        ...
        if ( ! $post instanceof WP_Post) {
            throw new RuntimeException(
                sprintf(
                    __('%s has already been deleted.', 'lenuaj-admin-columns'),
    ...
    protected function validate_post_lock($id)
    {
        ...
        if ($user_locked_post) {
            ...
            throw new RuntimeException(
                sprintf(
                    __('%s is currently being edited by %s.', 'lenuaj-admin-columns'),
