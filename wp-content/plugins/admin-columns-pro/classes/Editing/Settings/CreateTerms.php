...
class CreateTerms extends AC\Settings\Column {
	...
	public function create_view() {
		...
		if ( $taxonomy ) {
			$view->set( 'label', sprintf( __( 'Allow new %s?', 'lenuaj-admin-columns' ), strtolower( $taxonomy->labels->name ) ) )
			     ->set( 'tooltip', sprintf( __( 'Allow new %s to be created whilst editing', 'lenuaj-admin-columns' ), strtolower( $taxonomy->labels->name ) ) )
