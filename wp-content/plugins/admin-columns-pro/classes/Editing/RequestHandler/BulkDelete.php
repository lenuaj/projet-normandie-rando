...
class BulkDelete implements RequestHandler
{
    ...
    public function handle(Request $request)
    {
        ...
        if ( ! $list_screen instanceof Editing\BulkDelete\ListScreen ||
            ...
            $response->set_message(__('Table does not support bulk delete.', 'lenuaj-admin-columns'))
                     ->error();
        }
        ...
        if ( ! $deletable->user_can_delete()) {
            $response->set_message(__('Current user has no delete permissions.', 'lenuaj-admin-columns'))
                     ->error();
        }
