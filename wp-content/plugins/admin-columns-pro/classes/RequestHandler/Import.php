...
class Import implements RequestHandler
{
    ...
    public function handle(Request $request): void
    {
        ...
        if (empty($file_contents)) {
            $this->set_message(__('Uploaded file is empty or not readable.', 'lenuaj-admin-columns'));
        ...
        try {
            ...
            $this->set_message(__('Error parsing the uploaded file.', 'lenuaj-admin-columns'));
        ...
        if ( ! $list_screens->count()) {
            $this->set_message(
                __('The uploaded file does not contain any column settings.', 'lenuaj-admin-columns'),
                Message::WARNING
            );
    ...
    private function success(ListScreenCollection $list_screens): void
    {
        ...
        foreach ($grouped as $label => $links) {
            $message = sprintf(
                __('Successfully imported %s for %s.', 'lenuaj-admin-columns'),
