...
class LicenseActivate implements RequestHandler
{
    ...
    public function handle(Request $request): void
    {
        ...
        if ( ! $key) {
            $this->error_notice(__('Empty license key.', 'lenuaj-admin-columns'));

            return;
        }

        if ( ! LicenseKey::is_valid($key)) {
            $this->error_notice(__('Invalid license key.', 'lenuaj-admin-columns'));
        ...
        if ($response->has_error()) {
            ...
            if ($this->permission_storage->retrieve()->has_usage_permission()) {
                $this->succes_notice(
                    __('Product is activated, but automatic updates are disabled.', 'lenuaj-admin-columns')
                );
            }
