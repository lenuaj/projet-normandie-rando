...
class LockedSettings implements Registerable
{
    ...
    private function get_message(): string
    {
        return sprintf(
            ...
            sprintf(
                __("Go to the %s and activate Admin Columns Pro to start using the plugin.", 'lenuaj_admin_columns'),
                sprintf(
                    '<a href="%s">%s</a>',
                    esc_url($this->get_license_page_url()->get_url()),
                    __('license page', 'lenuaj_admin_columns')
                )
            )
        );
    }
    ...
    private function get_inline_plugin_message(): string
    {
        return sprintf(
            '%s %s',
            sprintf(
                __('%s is not yet activated, please %s.', 'lenuaj_admin_columns'),
                'Admin Columns Pro',
                sprintf(
                    '<a href="%s">%s</a>',
                    esc_url($this->get_license_page_url()->get_url()),
                    __('enter your license key', 'lenuaj_admin_columns')
                )
            ),
    ...
    {
        return sprintf(
            __('You can find your license key on your %s.', 'lenuaj-admin-columns'),
            sprintf(
                '<a href="%s" target="_blank">%s</a>',
                esc_url($this->get_account_url()->get_url()),
                __('account page', 'lenuaj-admin-columns')
            )
        );
    }
    ...
    private function get_license_page_message(): string
    {
        $documentation = sprintf(
            ...
            sprintf(__('activating %s', 'lenuaj-admin-columns'), 'Admin Columns Pro')
        );

        $parts = [
            __('To start using Admin Columns Pro, fill in your license key below.', 'lenuaj-admin-columns'),
