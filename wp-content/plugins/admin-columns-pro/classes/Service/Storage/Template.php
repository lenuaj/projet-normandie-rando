...
final class Template implements Registerable
{
    ...
    public function register_preview_notice(ListScreen $list_screen): void
    {
        ...
        $message = sprintf(
            '%s %s %s',
            sprintf(
                __('This is a preview of %s.', 'lenuaj-admin-columns'),
                "<strong>" . esc_html($list_screen->get_title()) . "</strong>"
            ),
            sprintf(
                __('Return to the %s page to import this table view.', 'lenuaj-admin-columns'),
                sprintf("<a href='%s'>%s</a>", esc_url($url->get_url()), __('Tools', 'lenuaj-admin-columns'))
            ),
            sprintf(
                '<a href="%s">%s</a>',
                esc_url($deactivate_url->get_url()),
                __('Leave preview mode', 'lenuaj-admin-columns')
            )
        );
