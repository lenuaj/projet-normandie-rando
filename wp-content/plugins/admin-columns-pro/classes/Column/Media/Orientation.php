...
class Orientation extends AC\Column\Media\Meta implements Export\Exportable
{
    ...
    public function __construct()
    {
        $this->set_type('column-orientation')
             ->set_group('media')
             ->set_label(__('Orientation', 'lenuaj-admin-columns'));
    }
    ...
    public function get_value($id)
    {
        ...
        if ($height === $width) {
            return _x('Square', 'image orientation', 'lenuaj-admin-columns');
        }

        return $width > $height
            ? _x('Landscape', 'image orientation', 'lenuaj-admin-columns')
            : _x('Portrait', 'image orientation', 'lenuaj-admin-columns');
    }
