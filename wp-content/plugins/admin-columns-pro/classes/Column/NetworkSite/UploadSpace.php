...
class UploadSpace extends AC\Column {
	...
	public function __construct() {
		...
		$this->set_label( __( 'Storage Space', 'lenuaj-admin-columns' ) );
	}
	...
	public function get_value( $id ) {
		...
		if ( $used ) {
			$display_used = sprintf( __( '%s MB', 'lenuaj-admin-columns' ), $this->trim_zeros( number_format_i18n( $used, 2 ) ) );
		}

		if ( $this->upload_restrictions() ) {
			$display_quota = sprintf( __( '%s MB', 'lenuaj-admin-columns' ), number_format_i18n( $quota ) );
		} else {
