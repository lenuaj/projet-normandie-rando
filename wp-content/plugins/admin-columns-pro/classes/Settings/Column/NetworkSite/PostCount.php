...
class PostCount extends Settings\Column
	...
	public function create_view() {
		...
		$view_post_type = new View( [
			'label'   => __( 'Post Type', 'lenuaj-admin-columns' ),
			'setting' => $setting,
			'for'     => $setting->get_id(),
		] );
		...
		if ( $excluded = $this->get_exludeded_post_statuses() ) {
			$setting->set_description( sprintf( __( 'Does not include %s', 'lenuaj-admin-columns' ), ac_helper()->string->enumeration_list( $excluded ) ) );
		}

		$view_post_status = new View( [
			'label'   => __( 'Post Status', 'lenuaj-admin-columns' ),
			'setting' => $setting,
			'for'     => $setting->get_id(),
		] );

		$view = new View( [
			'label'    => __( 'Display Options', 'lenuaj-admin-columns' ),
	...
	private function get_post_types() {
		...
		return [ '' => __( 'All post types', 'lenuaj-admin-columns' ) ] + $post_types;
	}
	...
	private function get_post_statuses() {
		...
		$options = [
			           ''              => __( 'Any post status', 'lenuaj-admin-columns' ),
			           'without_trash' => __( 'Any post status without Trash', 'lenuaj-admin-columns' ),
		           ] + $post_statuses;
