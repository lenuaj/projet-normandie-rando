...
class Theme extends Settings\Column
	...
	private function get_display_options() {
		$options = [
			'active'    => __( 'Active Theme', 'lenuaj-admin-columns' ),
			'allowed'   => __( 'Allowed Themes', 'lenuaj-admin-columns' ),
			'available' => __( 'Available Themes', 'lenuaj-admin-columns' ),
		];
	...
	public function format( $value, $original_value ) {
		...
		foreach ( $themes as $k => $theme ) {
			...
			if ( $theme->get_stylesheet() === $active_theme->get_stylesheet() ) {
				$tooltip[] = __( 'Active', 'lenuaj-admin-columns' );
			}

			if ( $theme->is_allowed( 'network', $blog_id ) ) {
				$tooltip[] = __( 'Network Enabled', 'lenuaj-admin-columns' );
			} else if ( $theme->is_allowed( 'site', $blog_id ) ) {
				$tooltip[] = __( 'Site Enabled', 'lenuaj-admin-columns' );
			}
