...
<div class="ac-tool-section -presets">
	<h2 class="ac-lined-header">
        <?= esc_html__('Import Templates', 'lenuaj-admin-columns') ?>
	</h2>
	<p>
        <?= esc_html__('Select the table view template you would like to import.', 'lenuaj-admin-columns'); ?>
	</p>
	<table class="widefat fixed ac-table -preset">
		<thead>
		<tr>
			<th class="list-table"><?= __('List Table', 'lenuaj-admin-columns') ?></th>
			<th class="name"><?= __('Name', 'lenuaj-admin-columns') ?></th>
			<th class="source"><?= __('Source', 'lenuaj-admin-columns') ?></th>
			<th class="actions">&nbsp;</th>
		</tr>
		</thead>
		<tbody>
				...
				<td class="actions">
					<form method="post">
						<a class="button" href="<?= esc_url($list_item['url_preview']) ?>">
                            <?= __('Preview', 'lenuaj-admin-columns') ?></a>
						...
						<button class="button button-primary"><?= __('Import', 'lenuaj-admin-columns') ?></button>
					</form>
