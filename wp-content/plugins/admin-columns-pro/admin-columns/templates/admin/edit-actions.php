...
<div class="sidebox form-actions">
	...
	<form class="form-reset" method="post">
		...
		<?php if ( $this->delete_confirmation_message ) : ?>
			<input class="reset-column-type" type="submit" value="<?php _e( 'Restore columns', 'lenuaj-admin-columns' ); ?>"
					onclick="return confirm( '<?php echo esc_js( $this->delete_confirmation_message ); ?>'); ">
		<?php else : ?>
			<input class="reset-column-type" type="submit" value="<?php _e( 'Restore columns', 'lenuaj-admin-columns' ); ?>">
		<?php endif; ?>

		<small class="list-screen-type"><?= sprintf( '%s: %s', __( 'Key', 'lenuaj-admin-columns' ), $this->list_screen_key ); ?></small>
		<small class="list-screen-id"><?= sprintf( '%s: %s', __( 'ID', 'lenuaj-admin-columns' ), $this->list_screen_id ); ?></small>
