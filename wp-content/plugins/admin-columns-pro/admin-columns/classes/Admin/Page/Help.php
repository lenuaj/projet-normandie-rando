...
class Help implements Enqueueables, Renderable, RenderableHead
{
    ...
    private function get_documention_link(): string
    {
        return sprintf(
            ...
            __('View documentation', 'lenuaj-admin-columns')
        );
    }
    ...
    {
        return sprintf(
            _n('The callback used is %s.', 'The callbacks used are %s.', count($callbacks), 'lenuaj-admin-columns'),
    ...
    private function render_actions(): void
    {
        ...
        ?>
		<h3><?php
            __('Deprecated Actions', 'lenuaj-admin-columns'); ?></h3>
        <?php

        foreach ($hooks as $hook) {
            $message = sprintf(
                __('The action %s used on this website is deprecated since %s.', 'lenuaj-admin-columns'),
    ...
    private function render_filters(): void
    {
        ...
        ?>
		<h3><?php
            __('Deprecated Filters', 'lenuaj-admin-columns'); ?></h3>
        <?php

        foreach ($hooks as $hook) {
            $message = sprintf(
                __('The filter %s used on this website is deprecated since %s.', 'lenuaj-admin-columns'),
                '<code>' . $hook->get_name() . '</code>',
                '<strong>' . $hook->get_version() . '</strong>'
            );
    ...
    public function render(): string
    {
        ...
        ?>
		<h2><?php
            _e('Help', 'lenuaj-admin-columns'); ?></h2>
		<p>
            <?php
            _e('The Admin Columns plugin has undergone some major changes in version 4.', 'lenuaj-admin-columns'); ?>
			<br/>
            ...
            printf(
                ...
                sprintf(
                    ...
                    (new Url\Documentation(Url\Documentation::ARTICLE_UPGRADE_V3_TO_V4))->get_url(),
                    __('our documentation', 'lenuaj-admin-columns')
                )
            );
            ?>
		</p>
        ...
        if ($this->hooks->get_count() > 0) {
            ...
            printf('<em>%s</em>', __('No deprecated hooks or filters found.', 'lenuaj-admin-columns'));
        }
