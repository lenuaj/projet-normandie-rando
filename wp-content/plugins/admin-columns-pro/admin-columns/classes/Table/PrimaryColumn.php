...
class PrimaryColumn {
	...
	public function set_media_download_row_action( array $actions, WP_Post $post ): array {
		$link_attributes = [
			'download' => '',
			'title'    => __( 'Download', 'lenuaj-admin-columns' ),
		];
		$actions['download'] = ac_helper()->html->link( wp_get_attachment_url( $post->ID ), __( 'Download', 'lenuaj-admin-columns' ), $link_attributes );
