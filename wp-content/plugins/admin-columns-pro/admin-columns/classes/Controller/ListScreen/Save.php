...
class Save
{
    ...
    public function request(Request $request): void
    {
        ...
        if ( ! isset($data['columns'])) {
            wp_send_json_error(['message' => __('You need at least one column', 'lenuaj-admin-columns')]);
        }
        ...
        if ( ! $this->list_screen_factory->can_create($list_key)) {
            wp_send_json_error(['message' => __('List screen not found', 'lenuaj-admin-columns')]);
        }
        ...
        if ( ! $this->storage->exists($list_id)) {
            wp_send_json_error(['message' => __('Column settings could not be saved.', 'lenuaj-admin-columns')]);
        }
        ...
        wp_send_json_success([
            'message' => sprintf(
                '%s %s',
                sprintf(
                    __('Settings for %s updated successfully.', 'lenuaj-admin-columns'),
                    sprintf('<strong>%s</strong>', esc_html($list_screen->get_title() ?: $list_screen->get_label()))
                ),
                ac_helper()->html->link(
                    (string)$list_screen->get_table_url(),
                    sprintf(__('View %s screen', 'lenuaj-admin-columns'), $list_screen->get_label())
                )
            ),
            'list_id' => $list_id->get_id(),
        ]);
