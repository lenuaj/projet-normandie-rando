...
class AvailableSizes extends Column\Media\MetaValue {
	...
	public function __construct() {
		$this->set_type( 'column-available_sizes' )
		     ...
		     ->set_label( __( 'Available Sizes', 'lenuaj-admin-columns' ) );
	}
	...
	public function get_value( $id ) {
		...
		if ( $available_sizes ) {
			...
			$paths[] = ac_helper()->html->tooltip( ac_helper()->html->link( $url, __( 'original', 'lenuaj-admin-columns' ) ), basename( $url ) );
		...
		if ( '1' === $this->get_setting( 'include_missing_sizes' )->get_value() ) {
			...
			if ( $missing ) {
				foreach ( $missing as $size ) {
					$paths[] = ac_helper()->html->tooltip( $size, sprintf( __( 'Missing image file for size %s.', 'lenuaj-admin-columns' ), '<em>"' . $size . '"</em>' ), [ 'class' => 'ac-missing-size' ] );
				}
			}
		}
