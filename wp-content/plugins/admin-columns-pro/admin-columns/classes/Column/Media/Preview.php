...
class Preview extends Column implements Column\AjaxValue {
	...
	public function __construct() {
		$this->set_type( 'column-preview' )
		     ->set_label( __( 'Preview', 'lenuaj-admin-columns' ) );
	}
	...
	public function get_ajax_value( $id ) {
		switch ( $this->get_media_type( $id ) ) {
			case 'audio':
				return sprintf( '<audio controls autoplay="autoplay" preload="none" src="%s">%s</audio>', esc_url( $this->get_raw_value( $id ) ), __( 'No support for audio player', 'lenuaj-admin-columns' ) );
			case 'video':
				return ( new Video( [] ) )
		...
		return __( 'Preview not available', 'lenuaj-admin-columns' );
	}
