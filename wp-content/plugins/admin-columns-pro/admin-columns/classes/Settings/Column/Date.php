...
class Date extends Settings\Column\DateTimeFormat {
	...
	private function get_diff_html_label() {
		$description = __( 'The difference is returned in a human readable format.', 'lenuaj-admin-columns' ) . ' <br/>' .
		               sprintf( __( 'For example: %s.', 'lenuaj-admin-columns' ),
			               '"' . $this->format_human_time_diff( strtotime( "-1 hour" ) ) . '" '
			               . __( 'or', 'lenuaj-admin-columns' ) .
			               ' "' . $this->format_human_time_diff( strtotime( "-2 days" ) ) . '"'
		               );

		return $this->get_html_label( __( 'Time Difference', 'lenuaj-admin-columns' ), '', $description );
	}

	protected function get_custom_format_options() {
		$options = [
			'diff'       => $this->get_diff_html_label(),
			'wp_default' => $this->get_default_html_label( __( 'WordPress Date Format', 'lenuaj-admin-columns' ) ),
		];
	...
	public function format_human_time_diff( $timestamp ) {
		...
		if ( $timestamp > $current_time ) {
			$tpl = __( 'in %s', 'lenuaj-admin-columns' );
		}
