...
class WordsPerMinute extends Settings\Column
	...
	public function create_view() {
		...
		$view = new View( [
			'label'   => __( 'Words per minute', 'lenuaj-admin-columns' ),
			'tooltip' => __( 'Estimated reading time in words per minute.', 'lenuaj-admin-columns' ) . ' ' . sprintf( __( 'By default: %s', 'lenuaj-admin-columns' ), $this->get_words_per_minute() ),
			'setting' => $setting,
		] );
	...
	protected function make_human_readable( $seconds ) {
		...
		if ( is_numeric( $seconds ) ) {
			...
			if ( $minutes < 1 ) {
				$time = $seconds . ' ' . _n( 'second', 'seconds', $seconds, 'lenuaj-admin-columns' );
			} else {
				$time .= ' ' . _n( 'minute', 'minutes', $minutes, 'lenuaj-admin-columns' );
			}
		}
