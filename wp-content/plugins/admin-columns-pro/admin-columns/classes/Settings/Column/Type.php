...
class Type extends Column
{
    ...
    public function create_view()
    {
        ...
        // Tooltip
        $tooltip = __('Choose a column type.', 'lenuaj-admin-columns');

        if (defined('WP_DEBUG') && WP_DEBUG) {
            $tooltip .= '<em>' . __('Type', 'lenuaj-admin-columns') . ': ' . $this->column->get_type() . '</em>';

            if ($this->column->get_name()) {
                $tooltip .= '<em>' . __('Name', 'lenuaj-admin-columns') . ': ' . $this->column->get_name() . '</em>';
            }
        }

        $args = [
            'setting' => $type,
            'label'   => __('Type', 'lenuaj-admin-columns'),
            'tooltip' => $tooltip,
        ];
