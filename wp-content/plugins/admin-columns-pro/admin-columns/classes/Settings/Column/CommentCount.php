...
class CommentCount extends Settings\Column
	...
	public function create_view() {
		...
		$view = new View( [
			'label'   => __( 'Comment status', 'lenuaj-admin-columns' ),
			'tooltip' => __( 'Select which comment status you like to display.', 'lenuaj-admin-columns' ),
			'setting' => $setting,
		] );
	...
	protected function get_comment_statuses() {
		$options = [
			self::STATUS_APPROVED => __( 'Approved', 'lenuaj-admin-columns' ),
			self::STATUS_PENDING  => __( 'Pending', 'lenuaj-admin-columns' ),
			self::STATUS_SPAM     => __( 'Spam', 'lenuaj-admin-columns' ),
			self::STATUS_TRASH    => __( 'Trash', 'lenuaj-admin-columns' ),
		];

		natcasesort( $options );

		// First
		$options = [ self::STATUS_ALL => __( 'Total', 'lenuaj-admin-columns' ) ] + $options;
