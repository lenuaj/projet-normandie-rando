...
abstract class DateTimeFormat extends Settings\Column
	...
	protected function get_default_html_label( $label, $date_format = '', $description = '' ) {
		...
		if ( ! $description && current_user_can( 'manage_options' ) ) {
			$description = sprintf(
				__( 'The %s can be changed in %s.', 'lenuaj-admin-columns' ),
				$label,
	...
	public function create_view() {
		...
		$view = new View( [
			...
			'label'               => __( 'Date Format', 'lenuaj-admin-columns' ),
			'tooltip'             => __( 'This will determine how the date will be displayed.', 'lenuaj-admin-columns' ),
		] );
	...
	protected function get_date_options() {
		...
		$custom_label = $this->get_html_label(
			__( 'Custom:', 'lenuaj-admin-columns' ),
			'',
			sprintf( __( 'Learn more about %s.', 'lenuaj-admin-columns' ), ac_helper()->html->link( 'https://wordpress.org/support/article/formatting-date-and-time/', __( 'date and time formatting', 'lenuaj-admin-columns' ), [ 'target' => '_blank' ] ) )
		);
