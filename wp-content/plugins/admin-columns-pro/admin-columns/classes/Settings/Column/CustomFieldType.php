    ...
    implements Settings\FormatValue
{
    ...
    public function create_view()
    {
        ...
        $tooltip = __('This will determine how the value will be displayed.', 'lenuaj-admin-columns');

        if ( ! in_array($this->get_field_type(), [null, ''], true)) {
            $tooltip .= '<em>' . __('Type', 'lenuaj-admin-columns') . ': ' . $this->get_field_type() . '</em>';
        }

        return new View([
            'label'   => __('Field Type', 'lenuaj-admin-columns'),
            'tooltip' => $tooltip,
            'setting' => $select,
        ]);
    }
    ...
    {
        $description = sprintf(
            __("Uses one or more %s IDs to display information about it.", 'lenuaj-admin-columns'),
            '<em>' . $input . '</em>'
        );
        $description .= ' ' . __("Multiple IDs should be separated by commas.", 'lenuaj-admin-columns');
    ...
    public function get_description()
    {
        ...
        switch ($this->get_field_type()) {
            case self::TYPE_POST :
                $description = $this->get_description_object_ids(__("Post Type", 'lenuaj-admin-columns'));

                break;
            case self::TYPE_USER :
                $description = $this->get_description_object_ids(__("User", 'lenuaj-admin-columns'));
    ...
    protected function get_field_type_options()
    {
        $grouped_types = [
            'basic'      => [
                self::TYPE_COLOR   => __('Color', 'lenuaj-admin-columns'),
                self::TYPE_DATE    => __('Date', 'lenuaj-admin-columns'),
                self::TYPE_TEXT    => __('Text', 'lenuaj-admin-columns'),
                self::TYPE_IMAGE   => __('Image', 'lenuaj-admin-columns'),
                self::TYPE_URL     => __('URL', 'lenuaj-admin-columns'),
                self::TYPE_NUMERIC => __('Number', 'lenuaj-admin-columns'),
            ],
            'choice'     => [
                self::TYPE_NON_EMPTY => __('Has Content', 'lenuaj-admin-columns'),
                self::TYPE_BOOLEAN   => __('True / False', 'lenuaj-admin-columns'),
            ],
            'relational' => [
                self::TYPE_MEDIA => __('Media', 'lenuaj-admin-columns'),
                self::TYPE_POST  => __('Post', 'lenuaj-admin-columns'),
                self::TYPE_USER  => __('User', 'lenuaj-admin-columns'),
            ],
            'multiple'   => [
                self::TYPE_COUNT => __('Number of Fields', 'lenuaj-admin-columns'),
                self::TYPE_ARRAY => sprintf(
                    '%s / %s',
                    __('Multiple Values', 'lenuaj-admin-columns'),
                    __('Serialized', 'lenuaj-admin-columns')
                ),
            ],
        ];
    ...
    private function get_grouped_options()
    {
        ...
        $groups = [
            'basic'      => __('Basic', 'lenuaj-admin-columns'),
            'relational' => __('Relational', 'lenuaj-admin-columns'),
            'choice'     => __('Choice', 'lenuaj-admin-columns'),
            'multiple'   => __('Multiple', 'lenuaj-admin-columns'),
            'custom'     => __('Custom', 'lenuaj-admin-columns'),
        ];
        ...
        $grouped_options = array_merge(['' => __('Default', 'lenuaj-admin-columns')], $grouped_options);
