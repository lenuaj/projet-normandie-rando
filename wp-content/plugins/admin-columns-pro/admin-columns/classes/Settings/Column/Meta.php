...
abstract class Meta extends Column
{
    ...
    protected function get_setting_field()
    {
        $setting = $this
            ...
            ->set_no_result(__('No fields available.', 'lenuaj-admin-columns'));
    ...
    {
        $view = new View([
            'label'   => __('Field', 'lenuaj-admin-columns'),
    ...
    protected function get_meta_groups()
    {
        ...
        $groups = [
            ''  => __('Public', 'lenuaj-admin-columns'),
            '_' => __('Hidden', 'lenuaj-admin-columns'),
        ];
        ...
        if (MetaType::USER === $this->get_meta_type()) {
            if (is_multisite()) {
                foreach (get_sites() as $site) {
                    $label = __('Network Site:', 'lenuaj-admin-columns') . ' ' . ac_helper(
                        )->network->get_site_option($site->blog_id, 'blogname');

                    if (get_current_blog_id() == $site->blog_id) {
                        $label .= ' (' . __('current', 'lenuaj-admin-columns') . ')';
                    }

                    $groups[$wpdb->get_blog_prefix($site->blog_id)] = $label;
                }
            } else {
                $groups[$wpdb->get_blog_prefix()] = __('Site Options', 'lenuaj-admin-columns');
            }
        }
