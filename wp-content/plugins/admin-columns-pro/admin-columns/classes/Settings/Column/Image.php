...
class Image extends Settings\Column
	...
	public function create_view() {
		$width = new View( [
			...
			'label'   => __( 'Width', 'lenuaj-admin-columns' ),
			'tooltip' => __( 'Width in pixels', 'lenuaj-admin-columns' ),
		] );

		$height = new View( [
			'setting' => $this->create_element( 'number', 'image_size_h' ),
			'label'   => __( 'Height', 'lenuaj-admin-columns' ),
			'tooltip' => __( 'Height in pixels', 'lenuaj-admin-columns' ),
		] );
		...
		$view = new View( [
			'label'    => __( 'Image Size', 'lenuaj-admin-columns' ),
			'setting'  => $size,
	...
	private function get_grouped_image_sizes() {
		...
		$sizes = [
			'default' => [
				'title'   => __( 'Default', 'lenuaj-admin-columns' ),
				'options' => [
					'thumbnail' => __( 'Thumbnail', 'lenuaj-admin-columns' ),
					'medium'    => __( 'Medium', 'lenuaj-admin-columns' ),
					'large'     => __( 'Large', 'lenuaj-admin-columns' ),
				],
			],
		];
		...
		if ( ! empty( $all_sizes ) ) {
			foreach ( $all_sizes as $size ) {
				...
				if ( ! isset( $sizes['defined'] ) ) {
					$sizes['defined']['title'] = __( 'Others', 'lenuaj-admin-columns' );
				}
		...
		$sizes['default']['options']['full'] = __( 'Full Size', 'lenuaj-admin-columns' );

		$sizes['custom'] = [
			'title'   => __( 'Custom', 'lenuaj-admin-columns' ),
			'options' => [ 'cpac-custom' => __( 'Custom Size', 'lenuaj-admin-columns' ) . '&hellip;' ],
		];
