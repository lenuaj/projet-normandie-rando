...
class PostFormatIcon extends Settings\Column
	...
	public function create_view() {
		...
		$view = new View( [
			'label'   => __( 'Use an icon?', 'lenuaj-admin-columns' ),
			'tooltip' => __( 'Use an icon instead of text for displaying.', 'lenuaj-admin-columns' ),
			'setting' => $setting,
		] );
	...
	public function format( $format, $post_id ) {
		...
		if ( $this->use_icon() ) {
			...
			$value = __( 'Standard', 'lenuaj-admin-columns' );
